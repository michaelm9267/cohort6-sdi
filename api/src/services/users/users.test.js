...
describe('users', () => {
  ...
  scenario('creates a user', async (scenario) => {
    const result = await createUser({
      input: {
        ...
        rank: 'String',
        password: 'String',
        shopID: scenario.user.two.shopID,
      },
    })

    expect(result.email).toEqual('String1974596')
    expect(result.firstName).toEqual('String')
    expect(result.lastName).toEqual('String')
    expect(result.rank).toEqual('String')
    expect(result.rank).toEqual('String')
    expect(result.password).toEqual('String')
    expect(result.shopID).toEqual(scenario.user.two.shopID)
  })
  ...
  scenario('updates a user', async (scenario) => {
    ...
    const result = await updateUser({
      id: original.id,
      input: { email: 'String55247002' },
    })

    expect(result.email).toEqual('String55247002')
  })
